<div class="tab-pane show active" id="all_roles">
    <div class="table-responsive" style="padding-top: 30px">
        <table class="table table-striped table-hover dataTable js-exportable" id="role_data" style="width: 100%;font-weight:600">
            <thead>
                <tr>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Name')}}</th>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Type')}}</th>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Actions')}} </th>
                </tr>
            </thead>
            <tbody>
            </tbody>
            <tfoot>
                <tr>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Name')}}</th>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Type')}}</th>
                    <th class="center-text" style="font-weight: 800">{{ trans('Dashboard.Actions')}} </th>
                </tr>
            </tfoot>
        </table>
    </div>
</div>
